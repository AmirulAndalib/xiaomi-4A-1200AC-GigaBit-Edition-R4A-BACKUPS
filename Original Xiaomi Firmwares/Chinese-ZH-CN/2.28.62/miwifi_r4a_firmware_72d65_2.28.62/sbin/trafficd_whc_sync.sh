...
wifi_parse()
{
    ...
    local ssid="`cat $cfgf | grep -w "ssid" | awk -F ":=" '{print $2}'`"
    local pswd="`cat $cfgf | grep -w "pswd" | awk -F ":=" '{print $2}'`"
...
guest_parse()
{
    ...
    [ -z "$disab" ] && disab=0
    local ssid="`cat $cfgf | grep -w "gst_ssid" | awk -F ":=" '{print $2}'`"
    local pswd="`cat $cfgf | grep -w "gst_pswd" | awk -F ":=" '{print $2}'`"
